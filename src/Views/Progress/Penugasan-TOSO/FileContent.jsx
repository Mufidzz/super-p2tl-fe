...
import {API_DATA_TO_SO, API_USER_WORK_TO_SO} from "../../../api/server";
...
const FileContent = ({setCount}) => {
    ...
    useEffect(() => {
        axios.get(API_DATA_TO_SO + "?core-only=true&pagination=" + CreatePaginationEncoding(pagination.count, pagination.offset) + "&filter=" + CreateFilterEncoding({"not_assigned_only" : true}) ).then((res) => {
            if (res.status === 200) {
                return res.data
            }
        }).then((json) => {
